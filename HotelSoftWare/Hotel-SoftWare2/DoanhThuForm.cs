...
namespace Hotel_SoftWare2
{
    public partial class DoanhThuForm : Form
    {
        ...
        {
            if (dtpNgayVao.Value < dtpNgaydi.Value && comboBoxTkTheo.Text != "")
            {
                if (comboBoxTkTheo.Text == "Chi tiết tiền thu")
                {
                    showTienPhongDetail(dgvTienPhongDetail);
                    showTienDvDetail(dgvTienDvDetail);
                    showTien();
                }
                else if (comboBoxTkTheo.Text == "Phòng")
                {
                    showTienTheoPhong(dgvTienPhongDetail, dgvTienDvDetail);
                    showTienTheoPhong();
                }
            }
            else
            {
                MessageBox.Show("Ngày không phù hợp hoặc còn để trắng loại thống kê!!!");
            }
        }
        ...
        private void showTienTheoPhong()
        {
            int sumtienDV = 0, sumtienPhong = 0, i;
            for (i = 0; i < dgvTienDvDetail.Rows.Count; i++)
            {
                sumtienDV += Convert.ToInt32(dgvTienDvDetail.Rows[i].Cells[1].Value);
            }

            for (i = 0; i < dgvTienPhongDetail.Rows.Count; i++)
            {
                sumtienPhong += Convert.ToInt32(dgvTienPhongDetail.Rows[i].Cells[1].Value);
            }
        ...
        void showTienTheoPhong(DataGridView dgv1, DataGridView dgv2)
        {
            htEntities context = new htEntities();
            dgv1.DataSource = context.ViewTienPhong(dtpNgayVao.Value, dtpNgaydi.Value);
            dgv2.DataSource = context.ViewTienDV(dtpNgayVao.Value, dtpNgaydi.Value);
            dgv2.AutoSizeColumnsMode = dgv1.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dgv2.AutoSizeRowsMode = dgv1.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;
        }

        private void btnXemTienKhiNhap_Click(object sender, EventArgs e)
        {
            ViewMoney vmn = new ViewMoney();
            vmn.Show();
        }
    }
}
